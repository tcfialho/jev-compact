...
test('restore state is only claimable once', async () => {
  ...
  const env = { CODEX_JEV_DATA_DIR: root };
...
test('installer preserves existing hooks and is idempotent', async () => {
  ...
  await installHooks('/repo/codex-jev-compact/dist/cli.js', env);
  await installHooks('/repo/codex-jev-compact/dist/cli.js', env);
  const config = JSON.parse(await readFile(file, 'utf8'));
  assert.equal(config.hooks.PreCompact.filter((x) => x.hooks.some((h) => h.command.includes('codex-jev-compact'))).length, 1);
  ...
  assert.match(config.hooks.PreCompact.at(-1).hooks[0].commandWindows, /codex-jev-compact/);
});
...
test('index restore mode injects compact index once, not full retained archive', async () => {
  ...
  const env = { CODEX_JEV_DATA_DIR: root, CODEX_JEV_RESTORE_MODE: 'index' };
...
test('PLUGIN_DATA wins and stale sidecars are swept', async () => {
  ...
  const env = { PLUGIN_DATA: join(root, 'plugin'), CODEX_JEV_DATA_DIR: other };
