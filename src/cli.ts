...
async function configureProvider(provider: Exclude<JevProvider, 'auto'>): Promise<void> {
  ...
  if (enabledPluginRoot()) await uninstallHooks();
  console.log(`Configured ${provider}.\nKey saved: ${saved.keyFile}\nProvider preference saved: ${saved.providerFile}`);
}

async function installAndExplain(cliPath = fileURLToPath(import.meta.url), commandPrefix = 'jevcomp'): Promise<void> {
...
async function main(): Promise<void> {
  ...
  if (cmd === 'configure') {
    ...
    if (provider !== 'typesafe' && provider !== 'openrouter') throw new Error('usage: jevcomp configure <typesafe|openrouter>');
  ...
  if (cmd === 'config') {
    ...
    if (args[0] === 'reset') { await resetUserSettings(process.env); console.log('Saved jevcomp settings reset to defaults.'); printSettings(userSettings(process.env)); return; }
    if (args.length < 2) throw new Error('usage: jevcomp config <mode|restore-mode|restore-max-chars|pin-recent-messages|loss-threshold|min-reduction-ratio> <value>');
  ...
  if (cmd === 'setup') {
    const provider = (args[0] ?? resolveProvider({ env: process.env })) as Exclude<JevProvider, 'auto'>;
    if (provider !== 'typesafe' && provider !== 'openrouter') throw new Error('usage: jevcomp setup [typesafe|openrouter]');
    if (!hasSavedProviderKey(provider, process.env)) await configureProvider(provider);
