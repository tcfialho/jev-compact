...
const detectedRoots = new Map<string, { root: string | null; at: number }>();
...
export function enabledPluginRoot(env: Env = process.env): string | undefined {
  ...
  if (known && Date.now() - known.at < DETECTION_TTL_MS) return known.root ?? undefined;
  const remember = (root: string | undefined) => { detectedRoots.set(home, { root: root ?? null, at: Date.now() }); return root; };
  remember(undefined);
  ...
  try {
    ...
    for (const plugin of installed ?? []) {
      ...
      if (existsSync(join(root, 'hooks', 'hooks.json'))) return remember(root);
    }
  } catch {
