...
export async function startDashboard(port = 43127, env = process.env): Promise<void> {
  const server = createServer(async (req: any, res: any) => {
    ...
    if (req.url === '/api/stats') { res.setHeader('content-type', 'application/json'); res.end(JSON.stringify(s)); return; }
    ...
    res.end(`<!doctype html><meta charset="utf-8"><title>jev-compact</title><style>body{font:16px system-ui;max-width:900px;margin:60px auto;padding:0 20px}strong{font-size:2rem}pre{background:#111;color:#eee;padding:16px;overflow:auto}table{border-collapse:collapse;width:100%}td,th{text-align:left;border-bottom:1px solid #ddd;padding:7px}</style><h1>jev-compact</h1><p><strong>${s.estimatedRestoreTokensAvoided.toLocaleString()}</strong> estimated reinjection tokens avoided</p><p>${s.compactions} compactions · ${s.restores} restores · ${s.failures} failures · ${s.jevInputTokens.toLocaleString()} measured Jev input tokens</p><h2>By tool</h2><table><thead><tr><th>Tool</th><th>Calls</th><th>Dropped</th><th>Truncated</th><th>Chars removed</th></tr></thead><tbody>${tools}</tbody></table><h2>Last event</h2><pre>${escapeHtml(JSON.stringify(s.last, null, 2))}</pre>`);
  });
