...
function num(env: Record<string, string | undefined>, key: string, fallback: number): number {
  const n = Number(env[key]); return Number.isFinite(n) ? n : fallback;
}
...
export async function handleHook(value: unknown, env: Record<string, string | undefined> = process.env): Promise<Record<string, unknown>> {
  ...
  if (input.hook_event_name === 'PostCompact') {
    ...
    if (state) await appendHistory({ at: new Date().toISOString(), sessionId: input.session_id, turnId: input.turn_id, trigger: input.trigger, model: input.model, provider: providerName(env), status: 'ready', stats: state.stats, decisions: state.decisions }, env);
    return { continue: true, suppressOutput: true };
  }
