...
async function main() {
    ...
    if (cmd === 'install') {
        const path = await installHooks(fileURLToPath(import.meta.url));
        console.log(`Installed hooks: ${path}\nOpen /hooks in Codex once to review/trust them.`);
        return;
    }
    if (cmd === 'uninstall') {
