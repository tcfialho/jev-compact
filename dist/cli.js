...
function agentArgs(args) {
    if (args.includes('all') || args.includes('both'))
        return ['codex', 'claude'];
    const picked = ['codex', 'claude'].filter((agent) => args.includes(agent));
...
async function chooseAgents(args) {
    ...
    if (requested)
        return requested;
    const found = ['codex', 'claude'].filter(hasCommand);
    if (!found.length)
        throw new Error('neither codex nor claude was found; install Codex or Claude Code first');
...
async function uninstall(args) {
    ...
    if (pluginRoot) {
        ...
        if (marketplace === 'jevcomp')
            execFileSync('codex', ['plugin', 'marketplace', 'remove', marketplace], { stdio: 'inherit', windowsHide: true });
    }
    else
