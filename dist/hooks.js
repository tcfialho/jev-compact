...
async function restore(input, event, env) {
    ...
    try {
        ...
        if (mode === 'preserve')
            payload = await injectableArchive(preview.messagesFile, preview.contextFile);
...
export async function handleHook(value, env = process.env) {
    ...
    if (input.hook_event_name === 'PreCompact') {
        ...
        try {
            ...
            const prepared = await prepareState({ sessionId: input.session_id, turnId: input.turn_id, trigger: input.trigger, model: input.model, createdAt, runId, stats: result.stats, decisions: result.decisions, index: renderIndex(result.messages, result.decisions, numAny(env, ['JEV_COMPACT_INDEX_MAX_CHARS', 'JEV_COMPACT_INDEX_CHARS'], 12_000)) }, renderMessages(result.messages), env, result.messages);
