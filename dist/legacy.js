...
export function adoptLegacyEnvironment(env) {
    for (const [name, value] of Object.entries(env)) {
        if (!name.startsWith('JEV_COMPACT_'))
            continue;
...
export function migrateLegacyConfig(env) {
    if (env.JEVCOMP_CONFIG_DIR)
        return;
    ...
    if (existsSync(current) || !existsSync(legacy))
        return;
    try {
        cpSync(legacy, current, { recursive: true, errorOnExist: false });
    }
    catch { }
}
export function legacyHistoryPaths(env) {
