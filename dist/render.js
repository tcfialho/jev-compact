...
export function renderMessagesForInjection(messages, resultHead = 8_000, resultTail = 2_000) {
    return render(messages, resultHead, resultTail);
}
function render(messages, resultHead, resultTail) {
    const out = [];
...
export function renderIndex(messages, decisions, maxChars = 12_000) {
    ...
    const lines = ['Jev-retained context index:'];
    ...
    if (protectedText.length) {
        ...
        for (const m of protectedText)
            lines.push(`- [${m.role}] ${m.text.length <= 500 ? m.text : `${m.text.slice(0, 499)}…`}`);
    }
    ...
    if (shown < kept.length)
        lines.push(`- … ${kept.length - shown} older retained calls omitted from this injected index; full retained context is on disk.`);
